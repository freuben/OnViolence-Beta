OnViolenceScore {var s, basicPath, headphones, <>score, globalMIDI, globalTimes, start, end, bufferTimes, selectPitch, buffType, algoTimes, <>stepPedal, <>chooseSamples, <>countPedalOn,<>countPedalOff,<>tempo=176, <>partials, <>partialNode, <percglobalTimesGlob,<>midiout, partialTrig=0, audioIn, document;

	*new {arg headOut=0,adjScore=13.75;
	^super.new.initOnViolenceScore(headOut,adjScore);
	}
	
	initOnViolenceScore {arg headOut=0,adjScore=13.75;
	s = Server.default;
	...
	Buffer.read(s, basicPath ++ "/AlgoScore/samples/hihat.wav", bufnum:7);
	Buffer.read(s, basicPath ++ "/AlgoScore/samples/woodblock.wav", bufnum:8); 
	Buffer.read(s, basicPath ++ "/AlgoScore/samples/ride.wav", bufnum:9);
	Buffer.read(s, basicPath ++ "/AlgoScore/samples/snarerim.aif", bufnum:10);
	Buffer.read(s, basicPath ++ "/AlgoScore/samples/conga.aif", bufnum:11);
	Buffer.read(s, basicPath ++ "/AlgoScore/samples/kick.wav", bufnum:12);
	Buffer.read(s, basicPath ++ "/AlgoScore/samples/snare1.wav", bufnum:13);
	Buffer.read(s, basicPath ++ "/AlgoScore/samples/cowbell.aif", bufnum:14);
	Buffer.read(s, basicPath ++ "/AlgoScore/samples/low_tom.wav", bufnum:15);
	Buffer.read(s, basicPath ++ "/AlgoScore/samples/splash.wav", bufnum:16);
	Buffer.read(s, basicPath ++ "/AlgoScore/samples/crash.aif", bufnum:17);
	Buffer.read(s, basicPath ++ "/AlgoScore/samples/ride_open.wav", bufnum:18);
	chooseSamples = [];
	...
	MIDIIn.polytouch = { arg src, chan, num, vel; 	
		case
		{num == 1} {this.startAudio(headOut);}
		{num == 2} {{this.displayScore(vel,adjScore);}.defer}
		{num == 3} {this.remove};
	};
	
	MIDIIn.sysrt = { arg src, chan, val;  tempo = val; };
	partials = PartialTracker(512, 5, 1, 2, 3);  //a partial tracker
	}
	
	startAudio {arg headphonesOut = 0;
	headphones = NodeProxy.audio(s, 2);
	headphones.play(headphonesOut);
	
	//if you want to hear an external midi take the comments off
	//audioIn = NodeProxy.audio(s, 2);
//	audioIn.play(headphonesOut);  
	
	partialNode = NodeProxy.audio(s, 1);
	partialNode.play(0); 
	...
	displayScore {arg page=1,scoreAdj=13.75;
	headphones.put(0, \headmonitor, 0, [\out, 0, \bufnum, 7, \vol, 0]);
	
	//for external midi:
	//audioIn.source = {AudioIn.ar([2,3])};
	
	partialNode.put(0, \numpar, 0,  [\fftbuf, 1, \magbuf, 2, \freqbuf, 3, \num, 5, \bus, 1]); //use AudioIn and track 5 loadest partials
	if(chooseSamples != [], {score = AlgorithmicScore.screenBounds;});
	score.noteAdjust = scoreAdj;
	this.funcPage(page);
	case
...
score.notes(selectPitch, (0,1..selectPitch.size), color);
headphones.spawn([\bufnum, buffType[step]+7]);
step = step + 1;
});
...
headphones.spawn([\bufnum, percbuffType[step]+7;]);
...
start {var percArray, pedalOnOld, pedalOffOld, pedalSwitch;
percArray = Array.fill(7, {rrand(0,9)});
MIDIIn.program = { arg src, chan, prog; 
midiout.program(chan,prog);
case
//pedals for turning pages:
{prog == 1} {stepPedal = stepPedal + 1;	{this.funcPage(stepPedal.postln);}.defer;
case
...
{prog == 6} {stepPedal = stepPedal - 1;	{this.funcPage(stepPedal);}.defer;}
...
//reads pedal numbers and arranges them
	...
	keyboardPedal {var percArray, pedalOnOld, pedalOffOld, pedalSwitch;
percArray = Array.fill(7, {rrand(0,9)});
...
case
//pedals for turning pages:
{keycode == 115} {stepPedal = stepPedal + 1;	{this.funcPage(stepPedal.postln);}.defer;
case
{[2,3,4].includes(stepPedal)} {countPedalOn = 0; countPedalOff = 1}
{stepPedal == 7} {countPedalOn = 4; countPedalOff = 5}
{stepPedal == 12} {countPedalOn = 16; countPedalOff = 16}
...
{keycode == 119} {stepPedal = stepPedal - 1;	{this.funcPage(stepPedal);}.defer;}
//pedals for sensors, algorithmic score, pages:
{keycode == 97} {
if(partialTrig == 0, {
countPedalOff = countPedalOff + 1;
...
SystemClock.clear;
	...
	*initClass {
	...
	signal = PlayBuf.ar(1, bufnum, 1);
	signal2 = signal*env;
	DetectSilence.ar(signal2);
